...
| `GET` | `/api/v1/projects/{project_id}/me/capabilities` | 是 | 已实现 | 查询当前用户在项目内的角色和 capabilities。 |
...
|---|---|---|
| `project_id` | integer | 项目内部编号。 |
| `user_id` | integer | 当前登录用户内部编号。 |
| `member_status` | string/null | 当前用户在该项目中的成员状态；非项目成员时为 `null`。 |
| `roles` | array[string] | 当前 active 项目角色绑定列表。disabled / removed 成员可能仍有历史 active role binding，因此前端不能用该字段做安全判断。 |
| `capabilities` | array[string] | 后端基于当前登录用户和项目重新计算出的可用能力。前端应以该字段判断按钮可用性。 |
...
```text
需要登录。该接口不要求用户已经是项目成员；非成员返回空 roles 和空 capabilities。
```
...
```text
1. capabilities 只从 active 项目成员 + active 项目级角色绑定 + active role_registry 计算。
2. `can_manage_system_users` 是系统级 capability，不会自动授予项目内 capabilities。
3. MVP 阶段 `can_manage_system_users` 由 `is_system_admin=true` 推导；后续可扩展为系统级角色绑定。
3. 前端展示角色时可读 roles，但所有按钮可用性应读 capabilities。
```
