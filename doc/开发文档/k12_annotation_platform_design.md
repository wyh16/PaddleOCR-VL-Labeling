...
15. 角色管理：系统用户、项目成员、项目角色、capabilities 和关键操作审计。
```
...
3. 回滚原因必须写入 history。
4. locked eval 的回滚需要 project_admin 或具备回滚 capability 的角色批准。
```
...
project_admin: 管理项目成员、标签注册表、关系注册表、锁定数据集、回滚版本
system_admin: MVP 阶段用于推导 can_manage_system_users，负责系统用户管理和全局初始化，不作为普通项目操作角色
viewer: 只读查看
```
...
```text
1. 用户账号是系统级对象。
2. 项目成员关系决定用户是否能进入某个项目。
3. 项目角色决定用户在该项目内能做什么。
4. 一个用户可以在不同项目拥有不同角色。
5. 前端只根据后端返回的 capabilities 展示入口，最终权限以后端校验为准。
6. 创建用户、禁用用户、授权、撤销角色、禁用成员、锁定、回滚、导出下载必须进入审计日志。
```

账号创建原则：

```text
1. 平台默认面向内网或实验室私有部署，不开放用户自助注册。
2. 用户账号只能由具备 can_manage_system_users 的管理员创建、导入或禁用。
3. project_admin 不能创建系统用户，只能把已有 active 用户加入自己有权限管理的项目。
4. 管理员创建用户时，后端只保存 password_hash，不保存、不返回、不记录明文密码。
5. 初始密码应作为一次性临时密码处理，首次登录后应要求用户修改密码；如果首次改密能力尚未实现，必须在部署说明中限制初始密码分发和轮换方式。
6. 用户创建、禁用和密码重置必须写入审计日志，记录操作者、目标 user_id、时间、IP 和操作结果。
```
...
13. 基础角色管理：can_manage_system_users 创建/禁用用户、项目成员、角色绑定和 capabilities 查询。
```
