export default {
  ...
  users: {
    ...
    selfRoleHint: '不能把当前登录的管理员账号降级为普通用户。',
    lastAdminHint: '当前账号是最后一个启用中的系统管理员，不能降级或禁用。',
    ...
    messages: {
      ...
      selfDemotionForbidden: '不能把当前登录的管理员账号降级为普通用户。',
      selfDisableForbidden: '不能禁用当前登录的管理员账号。',
      lastAdminProtected: '至少需要保留一个启用中的系统管理员。',
      enableSuccess: '用户已启用',
